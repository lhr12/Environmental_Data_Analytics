...
> ANSWER: The EPSG code is 4269 and it is a geographic coordinate system because it uses angular coordinate units. This is associated with the North American Datum 1983.
...
GageLoc <- read.csv('./Data/Raw/NWIS_SiteInfo_NE_RAW.csv')
...
> ANSWER: Dec_lat_va will have the latitutde and dec_long_va will have the longitude.
...
GageLoc_sf <- st_as_sf(GageLoc,coords = c("dec_long_va","dec_lat_va"),crs=4269)
...
> ANSWER: Dec_lat_va and dec_long_va were replaced by a new column called geometry that contained the coordinates.
...
ggplot() + 
  ...
  geom_sf(data = GageLoc_sf,col='blue') +
...
SiteFlow_join <- SiteFlow %>% 
  left_join(y = GageLoc,by = "site_no") %>%
  na.omit()

#Convert back to sf dataframe
SiteFLow_join_sf <- st_as_sf(SiteFlow_join,coords = c("dec_long_va","dec_lat_va"),crs=4269)
```
...
#Plot the values
ggplot() +
  geom_sf(data = counties_sf, color = "dark red") +
  geom_sf(data = SiteFLow_join_sf, aes(color = gage_ht)) +
  labs(x = "Longitude", y = "Latitude", color = "Gage Height (ft)")
```
...
SiteFlow_utm <- st_transform(SiteFLow_join_sf, crs = 32614)
#Plot the data
ggplot() +
  geom_sf(data = counties_utm, color = "dark red") +
  geom_sf(data = SiteFlow_utm, aes(color = gage_ht)) +
  labs(x = "Longitude", y = "Latitude", color = "Gage Height (ft)")
```
> QUESTION: The shape of Nebraska should look a bit different than the one created in Step 5? Why?
> ANSWER: The shape is using a projected coordinate system instead of a geographic coordinate system.
...
#Select gages within 
Lancaster_mask <- st_within(SiteFlow_utm, 
                                Lancaster,
                                sparse = FALSE)
...
ggplot() +
  ...
  geom_sf(data = SiteFlow_utm[Lancaster_mask,], aes(color = gage_ht)) + 
  labs(x = "Longitude", y = "Latitude", color = "Gage Height (ft)")
...
#Join features
Gage_Counties_Join <- st_join(SiteFlow_utm, counties_utm, join = st_intersects)

#Show column names
colnames(Gage_Counties_Join)

#Plot
ggplot() +
  geom_sf(data = counties_utm) +
  geom_sf(data = Gage_Counties_Join, aes(color = NAME, size = gage_ht)) +
  labs(x = "Longitude", y = "Latitude", color = "County", size = "Gage Height (ft)")
```
...
Gage_sum_simple <- st_drop_geometry(Gage_sum)

#Join summary to County fc
Gage_sum_join <- Gage_sum_simple %>% 
  left_join(y = counties_utm,by = "NAME")

#Plot
ggplot() +
  geom_sf(data = counties_utm) +
  geom_sf(data = Gage_sum_join, aes(fill = gage_ht)) +
  labs(x = "Longitude", y = "Latitude", fill = "Gage Height (ft)")
