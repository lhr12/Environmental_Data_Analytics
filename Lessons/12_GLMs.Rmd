...
# another way to format the lm function. make sure y is on left side of tilda and x is on right side
irradiance.regression <- lm(data = PeterPaul.chem.nutrients, irradianceWater ~ depth)
summary(irradiance.regression)
# irradiance = 487 - 96(depth)
#only explaining 31% of variance from depth
# adjusted R squared penalizes you for adding more variables, 
...
> ANSWER: The irradiance is predicted to be 487 units at the surface, and decreasing by 96 units with every unit increase in depth.  Irradiance is negatively correlated with depth (linear regression; R2 = 0.31, df = 15449, p < 0.001).
*for a linear regression, we want to report R squared rather than F-statistic
*geom_text to add linear regression equation to graph
...
```{r, fig.height = 3, fig.width = 4}
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, irradianceWater != 0) #remove 0s b/c can't be log-transformed
...
#ln(irradiance) = 6.2 - 0.74(depth)

# Add a line and standard error for the linear regression
irradiancebydepth2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
  geom_smooth(method = "lm") +
  scale_y_log10() + #y breaks on a log scale rather than a linear scale
  geom_point() 
...
irradiancebydepth2 <- 
    ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
    geom_point(alpha = 0.5) + #Making points transparent
    scale_y_log10() +
  scale_x_continuous(breaks = c(0,2,4,6,8,10,12)) +
  labs(x = "Depth (m)", y = "Irradiance (units)") +
    geom_smooth(method = 'lm', se = FALSE, color = "pink") +
  geom_text(expression("ln(irradiance) = 6.2 - 0.74(depth)"))
print(irradiancebydepth2)
```
...
#tp_ug = 6 + 1.5(depth) + 0.9(DO)  
#intercept = 0 depth and 0 DO
#only explaining 8% of variance in data
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>%
  na.omit() #can't have any NAs to run a correlation plot
...
step(TPAIC)  #Start at full linear model, can build up with increase = TRUE
#AIC within each step = if I remove this variable, what will my AIC be?
...
# tn_ug = 353 (for paul lake at zero depth) + 135(for peter lake, so 488) -9.7*(each additional unit of depth)

# interaction effects
TNancova.interaction <- lm(data = PeterPaul.chem.nutrients, tn_ug ~ lakename * depth)
summary(TNancova.interaction)
# 325 (paul lake at 0 depth) + 185 (peter lake) + 20 (if it's paul&depth) - 48(if it's peter&depth)
