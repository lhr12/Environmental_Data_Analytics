...
#null hypothesis: no trend over time. The farther away z is from 0, the stronger the trend
#because z is positive, there is a positive trend
```
...
pettitt.test(Peter.nutrients.surface$tp_ug[35:132]) #at 35 + 79
...
ggplot(PeterPaul.nutrients.surface, aes(x = sampledate, y = tp_ug, color = lakename)) + 
  geom_point() +
  geom_vline(xintercept = as.Date("1993-05-26"), linetype = 2)+
  geom_vline(xintercept = as.Date("1998-05-26"), linetype = 2)+
  scale_color_manual(values = c("#7fcdbb", "#253494"))
...
USGS.flow.data.timeseries <- ts(USGS.flow.data.interpolated$Discharge, #ts turns dataframe into time series
