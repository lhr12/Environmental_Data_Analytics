...
2. **R Code Chunks** surrounded by ``` on top and bottom
    + Create using `Cmd/Ctrl` + `Alt` + `I` ('control', 'option', 'I' on Macs)
...
- Focus on output and conclusions, not code (or vice versa) (flexible formatting)
...
Dataset | Description
--------|--------------------------------------------------
Ecotox  | Mortality of organisms from Neonicotinoids
EPA Air | O3 and PM2.5 levels from 2017 and 2018
NTL-LTER| Nutrient and Chemical data for lakes in Wisconsin
USGS    | Flow rates from Eno River Spring 2019
...
```{r}
Nutrients_Summary_N <- Nutrients %>%
  filter(!is.na(tn_ug)) %>%
  group_by(lakename) %>%
  summarize(tn_mean = mean(tn_ug), tn_min = min(tn_ug), 
            tn_max = max(tn_ug), tn_std = sd(tn_ug))
 
Nutrients_Summary_P <- Nutrients %>%
  filter(!is.na(tp_ug))  %>%
  group_by(lakename) %>%
  summarize(tp_mean = mean(tp_ug), tp_min = min(tp_ug), 
            tp_max = max(tp_ug), tp_std = sd(tp_ug)) 
```
...
```{r, echo=FALSE}
kable(x = Nutrients_Summary_N, format = "latex")
kable(x = Nutrients_Summary_P, format = "latex")
```
