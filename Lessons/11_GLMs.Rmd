...
shapiro.test(EPAair$Ozone) #if the p-value is less than 0.05, the dataset is not normally distributed
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram(stat = "count") #frequently see long right tails in environmental datasets
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone) #if points not on line, show departure from normality
#run one-sample t-test because points do follow line for most of distribution

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less")
O3.onesample #low p-value => reject null hypothesis, t score => further from zero, lower probability will be
```

What information does the output give us? How might we report this information in a report?

> ANWSER:
For the years 2017-2018, the mean ozone AQI values were significantly lower than 50 (one sample t-test; t = -41.9, df = 1084, p <0.0001). We are 95% confident that the true mean lies between negative infinity and 37.43.
...
var.test(EPAair$Ozone ~ EPAair$Site.Name) #now that we are comparing to populations, want to see if variances similar or not, if p<0.05, the variances are not the same
...
O3.twosample #reject the null hypothesis that the means were the same for the two groups
...
bartlett.test(EPAair$PM2.5 ~ EPAair$Site.Name) #var.test will only do two at a time, this will do more than 2
...
TukeyHSD(PM2.5.anova2) #can only run tukeyHSD on an aov, not lm; look at p values to see which ones are significantly different from each other
...
> ANSWER: Want to show that blackstone and bryson city are different, blackstone and triple oak are different, but bryson and triple oak are not different. Blackstone has higher PM 2.5 values than bryson and triple oak (ANOVA; F = 16.16, df = 1898, p <0.0001)
