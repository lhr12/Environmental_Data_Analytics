...
* Aesthetics must be specified, most commonly x and y variables but including others. Aesthetics can be specified in the `ggplot()` function or in subsequent layers. #can define color, x/y labels, etc
...
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone), color = "blue") #can't define color within aesthetics
print(O3plot2)
...
PMplot.faceted <-
  ...
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) #if you don't specify nrow, will organize in columns
print(PMplot.faceted)

PMplot.faceted2 <-
  ggplot(EPAair, aes(x = month, y = PM2.5)) +
  geom_point() +
  facet_grid(Site.Name ~ year) #~ means "by"
print(PMplot.faceted2)

# Filter dataset within plot building
O3plot.Blackstone <- 
  ggplot(subset(EPAair, Site.Name == "Blackstone"), aes(x = Date, y = Ozone)) + #have to put quotes around Blackstone in subset
  geom_point() +
  geom_line() #good for seeing swings between points
...
NvsP <-
  ...
  geom_point() +
  geom_smooth(method = lm) + #need method = linear model to make it straight line, otherwise will just try to minimize error and line gets curvy
  geom_abline(aes(slope = 16, intercept = 0), lty = 2) #lty = line type, outside aes
print(NvsP)
...
PeterPaul.nutrient.summaries <- PeterPaul.nutrients.gathered %>%
  group_by(lakename, nutrient) %>% #if you want it to group by multiple different factors, you have to do it in the same function.  if you do it seperately, the lowest one will override the previous ones
  summarise(sd = sd(concentration), 
            mean = mean(concentration))

Nutrientplot <- 
  ggplot(PeterPaul.nutrients.gathered) +
  geom_bar(aes(x = lakename, y = concentration, fill = as.factor(nutrient)), # why did we use fill? will border bars in colors, but inside will be black if we use color = 
           position = "dodge", stat = "summary", fun.y = "mean")             # what's happening here? position = "dodge" spreads out into multiple bars, otherwise would be stacked
print(Nutrientplot)

Nutrientplot2 <- 
  ggplot(PeterPaul.nutrient.summaries, aes(x = lakename, y = mean, fill = as.factor(nutrient))) + #
  geom_bar(stat = "identity", position = "dodge") + # what does the stat command do? since we had already calculated the mean in our summary dataset, so we will just take the identity of whatever cell it's pointing at
  geom_errorbar(aes(ymin = mean-sd, ymax = mean+sd),position = "dodge") # how do we specify error bars? sd and mean are already part of dataset, would otherwise have to define something that represented mean and stdev
print(Nutrientplot2)

# Are there more effective ways to produce summary stats for categories? error bars completely inaccurate, nutrient levels cannot go below zero
...
print(Nutrientplot6) #if you don't specify colors, they will all be black
