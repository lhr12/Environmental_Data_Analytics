...
```{r}
# Load package
library(tidyverse) #using library will tell you if there are any issues loading that package, require will not tell you about any loading issues
```
...
dim(USGS.flow.data) #dimensions rows and columns
length(USGS.flow.data) #number of columns

head(USGS.flow.data) #shows first couple of rows of each column
head(USGS.flow.data, 10) 
tail(USGS.flow.data, 5) #shows last five rows of each column
USGS.flow.data[30000:30005, c(3, 8, 14)] #give me rows 30000 to 30005 and columns 3, 8, and 14

class(USGS.flow.data$datetime)
class(USGS.flow.data$discharge.mean)
class(USGS.flow.data$gage.height.mean)

summary(USGS.flow.data) #gives some basic statistics about numerical data
...
> ANSWER: they were registered as NAs
