...
class(NTL.phys.data.PeterPaul$sampledate) #R loses memory of class you had set before when you reload csv's
...
#filter chooses rows, select chooses columns
...
#for numeric, will fill in with NA if blank, will not for factors
#drop.levels will remove memory from original datset
...
#if you want to remove columns using select, put a minus sign next to column name 
...
#removing NAs associated with concentration, counts of each measurement will be different
...
#will replace original column, if you want to keep original use mutate
...
#have to specifiy which one is first and which one is second
...
#will always add to end of dataset
...
#not reproducible if we change datset, better to do it this way:
...
NTL.PeterPaul.summaries <- 
  ...
  filter(depth == 0) %>%  #choose only the surface depths
  group_by(lakename) %>%  #separate lake name peter and paul
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>%  #only took rows with measurements with all three
