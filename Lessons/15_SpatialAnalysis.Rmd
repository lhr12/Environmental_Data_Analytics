...
myMap  <- leaflet(data=EPAair_PM_avg) %>% 
  addTiles() %>% #tiles are background maps, currently using default
...
Durham_buffer <- EPAair_PM_avg_sf %>% 
  ...
  st_buffer(0.1) #units of 0.1 degrees
...
mapview(huc8_sf['SUB'], 
        graticule = st_crs(huc8_sf), #graticule: geographic grid in background
        axes = TRUE)
```
...
state_sf <- counties_sf %>% 
  group_by(STATEFP) %>% 
  summarize(ALAND = sum(ALAND)) #creates dataframe

#View the data
mapview(state_sf)

#Alternatively, we could use the `st_union` function
state_sf2 <- st_union(counties_sf) #creates list
mapview(state_sf2)
```
...
leaflet() %>% addTiles() %>%  #leaflet can only use WGS84
...
#ordering of features important, put base map first and then things on top of that afterwards
...
theSite_sfp <- st_point(c(userLng,userLat)) #sfp = spatial feature point

#Create a simple features column from the point geometry object
theSite_sfc <- st_sfc(theSite_sfp, crs = 4326) #sfc = spatial feature column
