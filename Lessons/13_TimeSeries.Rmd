...
TempTest.auto <- lme(data = PeterPaul.summertemp,
                     temperature_C ~ sampledate * lakename, #fixed effects portion of model w/ interaction
                     random = ~1|Week) #random effect, each week has its own intercept in model
TempTest.auto
#random effects intercept tells us the variability of temperature across the season among all of the weeks

ACF(TempTest.auto)
#autocorrelation value of 0.42, therefore 42% of variation due to autocorrelation
```
...
ggplot(PeterPaul.summertemp, aes(x = sampledate, y = temperature_C, color = lakename)) +
  ...
  labs(x = "Year", y = "Temperature (˚C)", color = "Lake") 
